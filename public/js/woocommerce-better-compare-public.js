	...
	$.extend( Plugin.prototype, {
			...
			if(!that.isEmpty(urlProducts)) {
				...
				$.each($(that.settings.addToCompareLink), function(i, index) {
					...
					if(typeof(that.products[product_id]) != "undefined") {
						$this.removeClass('add-to-compare-button').addClass('remove-from-compare-button').attr("disabled", true).html(that.trans.remove);
						console.log('beloooo - remove spinner and activate check - each loop');
		...
		compareBarAdd : function() {
			...
			$(document).on('click', that.settings.addToCompareLink, function(e) {
				...
				$this.closest('#belo_baba').prepend('<i  style=" top: 15px; position: absolute; left: 36px;" class="fa fa-refresh fa-spin"></i>');

				// is already on list
				if(typeof(that.products[product_id]) != 'undefined') {
					
					$this.removeClass('loading add-to-compare-button').addClass('remove-from-compare-button').html(that.trans.remove);
					$this.closest('#belo_baba').find('i.fa-spin').remove();
				...
				//belothat.getSingleProduct(product_id);
				jQuery.ajax({
					url: that.settings.ajax_url,
					type: 'post',
					dataType: 'JSON',
					...
					success : function( response ) {    
						...
						$('.add-to-compare-button[data-product-id="' + product_id + '"]').removeClass('loading add-to-compare-button').addClass('remove-from-compare-button').attr("disabled", true).html(that.trans.remove);
						//$('.add-to-compare-button[data-product-id="' + product_id + '"]').attr("disabled", true);
					},
					error: function(jqXHR, textStatus, errorThrown) {
		...
		compareBarRemove : function() {
			...
			$('.woocommerce-single-compare-table').on('click', that.settings.compareItemRemoveLink, function(e) {
				...
				$(document).find('input[data-product-id="' + product_id + '"]').prop('checked', false);
				$(document).find('input[data-product-id="' + product_id + '"]').closest('#belo_baba').find('i.fa-spin').remove();
				console.log('beloooo - deactivate check [table remove]');
			...
			$('.woocommerce-compare-bar-items').on('click', that.settings.compareBarRemoveLink, function(e) {
				...
				$(document).find('input[data-product-id="' + product_id + '"]').prop('checked', false).attr("disabled", false);
				$(document).find('input[data-product-id="' + product_id + '"]').closest('#belo_baba').find('i.fa-spin').remove();
				console.log('beloooo - deactivate check [compare bar remove]');
		...
		getSingleProduct: function(product_id, isSaved) {
			...
			jQuery.ajax({
				...
				success : function( response ) {
					...
					//Belo$('.add-to-compare-button[data-product-id="' + product_id + '"]').removeClass('loading add-to-compare-button').addClass('remove-from-compare-button').html(that.trans.remove);
				},
				error: function(jqXHR, textStatus, errorThrown) {
		...
		addProductToBar : function(product, isSaved) {
			...
			if(emptyBarItemContainers.length > 0) {
				...
				$('input[data-product-id="' + product_id + '"]').closest('#belo_baba').find('i.fa-spin').remove();
				console.log('beloooo - remove spinner and activate check');
 				$('input[data-product-id="' + product_id + '"]').prop("checked",true);
		...
		clearAll : function () {
			...
			that.settings.clearAllLink.on('click', function(e) {
				...
				$.each(that.products, function(i, product_id) {
					...
					//Belo tweak - loop clear check boxes
					$('input[data-product-id="' + product_id + '"]').prop("checked",false).attr("disabled", false);
					var barContainer = $('.woocommerce-compare-bar-items').find('.woocommerce-compare-bar-item-container[data-product-id="' + product_id + '"]');
					barContainer.addClass('woocommerce-compare-bar-item-placeholder').removeData('data-product-id');
					barContainer.find('.woocommerce-compare-bar-item').html('');
